...
// CODE HERE
const coolName = (me, myself) => `the first paramiter is ${me}. The second parameter is ${myself}`


// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

console.log(coolName(`cool`, `dude`))
...
const greeting = (firstName, lastName, callback) => {
    callback = `Hello, my full name is ${firstName} ${lastName}`
    return callback
}

// INVOKE 'GREETING FUNCTION' HERE

console.log(greeting(`Mr. Megatron`, `Rodgers`))
...
const pricesPlusTax = (prices, callback) =>{
    for(let i = 0; i < prices.length; i++){
        callback = Math.floor(prices[i] * .2 + prices[i])
        totalCost.push(callback)
    }return callback
}
/* 
